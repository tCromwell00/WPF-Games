...
namespace SolitaireGames.KlondikeSolitaire
{
    ...
    public class KlondikeSolitaireViewModel: CardGameViewModel
    {
        ...
        public KlondikeSolitaireViewModel()
        {
            ...
            if (Apex.Design.DesignTime.IsDesignTime) DoDealNewGame(null);
        ...
        {
            //  Call the base, which stops the timer, clears the score etc.
            base.DoDealNewGame(parameter);

            //  Clear everything.
            stock.Clear();
            waste.Clear();
            foreach (var tableau in tableaus) tableau.Clear();
            foreach (var foundation in foundations) foundation.Clear();

            //  Create a list of card types.
            List<CardType> eachCardType = new List<CardType>();
            foreach (CardType cardType in Enum.GetValues(typeof(CardType))) eachCardType.Add(cardType);

            //  Create a playing card from each card type.
            List<PlayingCard> playingCards = new List<PlayingCard>();
            foreach (var cardtype in eachCardType) playingCards.Add(new PlayingCard() { CardType = cardtype, IsFaceDown = true });
        ...
        private void DoTurnStock()
        {
            ...
            else
            {
                ...
                foreach (var wasteCard in waste) wasteCard.FaceUpOffset = 0;
                //  Work out how many cards to draw.
                int cardsToDraw = 0;
                switch (DrawMode)
                {
                    case DrawMode.DrawOne: cardsToDraw = 1; break;
                    case DrawMode.DrawThree: cardsToDraw = 3; break;
                    default: cardsToDraw = 1; break;
                }
            ...
            foreach (var wasteCard in waste) wasteCard.IsPlayable = wasteCard == Waste.Last();
        }

        /// <summary>
        /// Tries to move the card to its appropriate foundation.
        /// </summary>
        ...
        public bool TryMoveCardtoAppropriateFoundation(PlayingCard card)
        {
            ...
            if (waste.LastOrDefault() == card)
            {
                ...
                {
                    if (MoveCard(waste, foundation, card, false)) return true;
                }                
            }
            ...
            int i = 0;
            for (; i < tableaus.Count && inTableau == false; i++) inTableau = tableaus[i].Contains(card);

            //  If it's not in a tableau and not on the top
            //  of the waste, we cannot move it.
            if (inTableau == false) return false;

            //  Try and move to each foundation.
            foreach (var foundation in foundations)
            {
                if (MoveCard(tableaus[i - 1], foundation, card, false)) return true;
            }
        ...
        /// <summary>
        /// Tries to move all cards to appropriate foundations.
        /// </summary>
            ...
            if (from == Waste)
            {
                ...
                else if (tableaus.Contains(to))
                {
                    ...
                    if ((to.Count == 0 && card.Value == 12) ||
                        (to.Count > 0 && to.Last().Color != card.Color
        ...
        private void DoMoveCard(ObservableCollection<PlayingCard> from, 
            ...
            PlayingCard card)
        {
            //  Identify the run of cards we are moving.
